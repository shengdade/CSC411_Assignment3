...
def extract_inception_val():
    ...
    X_inception_val = np.zeros((val_size, 2048, 8, 8))
    ...
    for i in range(val_size):
        ...
        x = preprocess_input(x)
        inception = model.predict(x)
        X_inception_val[i, :, :, :] = inception
        print('Read image: ' + X_filelist[i])

    return X_inception_val
...
def save_inception(save_path='../../411a3/train_inception.npz'):
    X_train, y_train = extract_inception()
    train = {
        'X_train': X_train,
        'y_train': y_train
    }
...
def save_inception_val(save_path='../../411a3/train_inception_val.npz'):
    X_val = extract_inception_val()
    val = {
        'X_val': X_val
    }
    Save(save_path, val)
...
def save_inception_test(save_path='../../411a3/train_inception_test.npz'):
    X_test = extract_inception_test()
    test = {
        'X_test': X_test
    }
    Save(save_path, test)
...
def load_inception(nb_test=0, load_path='../../411a3/train_inception.npz'):
...
def load_inception_val(load_path='../../411a3/train_inception_val.npz'):
    val = Load(load_path)
    X_val = val['X_val']
...
def load_inception_test(load_path='../../411a3/train_inception_test.npz'):
    test = Load(load_path)
    X_test = test['X_test']
...
def create_model(input_shape):
    model = Sequential()
    model.add(AveragePooling2D((8, 8), strides=(8, 8), name='avg_pool', input_shape=input_shape))
...
def main():
    ...
    (X_train, y_train), (X_test, y_test) = load_inception()
    ...
    X_val = load_inception_val()
    X_test = load_inception_test()
